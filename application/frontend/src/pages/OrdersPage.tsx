  ...
  return (
    <>
      ...
      <Card className="overflow-hidden" aria-busy={orders.isPending}>
        <div className="grid gap-4 border-b border-slate-200 bg-slate-50/70 px-6 py-5 md:grid-cols-[1fr_240px]">
          <div className="relative self-end"><Search className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input dir="auto" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search customer or product" aria-label="Search orders" className="min-h-11 w-full rounded-xl border border-slate-300 bg-white pl-10 pr-4 text-sm shadow-sm transition-colors hover:border-slate-400 focus:border-brand-600 motion-reduce:transition-none" /></div>
        ...
        {orders.data && orders.data.results.length > 0 ? (
          <>
            <div className="table-scroll overflow-x-auto"><table className="w-full min-w-[820px] text-left"><thead className="border-y border-slate-200 bg-slate-100/80 text-[11px] font-bold uppercase tracking-[0.12em] text-slate-600"><tr><th scope="col" className="px-6 py-4">Order</th><th scope="col" className="px-6 py-4">Customer</th><th scope="col" className="px-6 py-4 text-right">Items</th><th scope="col" className="px-6 py-4 text-right">Amount</th><th scope="col" className="px-6 py-4">Status</th><th scope="col" className="px-6 py-4">Created</th></tr></thead><tbody className="divide-y divide-slate-200/70">{orders.data.results.map((order) => { const total = order.items.reduce((sum, item) => sum + Number(item.unit_price) * item.quantity, 0); return <tr key={order.id} role="link" tabIndex={0} aria-label={`Open order ${order.id}`} onClick={(event) => handleRowClick(event, order.id)} onKeyDown={(event) => handleRowKeyDown(event, order.id)} className="cursor-pointer transition-colors duration-150 hover:bg-brand-50/40 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-brand-500/60 motion-reduce:transition-none"><td className="px-6 py-4.5"><Link to={orderPath(order.id)} className="font-bold text-brand-600 hover:text-brand-700">#{order.id}</Link></td><td className="px-6 py-4.5"><p className="text-sm font-semibold text-slate-900">{order.user.username}</p><p className="mt-0.5 text-xs text-slate-500">{order.user.email || "No email"}</p></td><td className="px-6 py-4.5"><div className="flex items-center justify-end gap-2"><div className="flex -space-x-2" aria-hidden="true">{order.items.slice(0, 3).map((item) => <ProductImage key={item.id} src={item.product.image} size="md" className="ring-2 ring-white" />)}</div><span className="text-sm tabular-nums text-slate-600">{order.items.length} line{order.items.length === 1 ? "" : "s"}</span></div></td><td className="px-6 py-4.5 text-right text-sm font-bold tabular-nums text-slate-800">{formatCurrency(total)}</td><td className="px-6 py-4.5"><StatusBadge status={order.status} /></td><td className="px-6 py-4.5 text-sm text-slate-500">{formatDate(order.created_at)}</td></tr>; })}</tbody></table></div>
            <div className="border-t border-slate-200 bg-slate-50/50 px-6 py-4"><Pagination page={page} count={orders.data.count} hasNext={Boolean(orders.data.next)} hasPrevious={Boolean(orders.data.previous)} onPageChange={setPage} /></div>
          </>
        ) : null}
      </Card>
