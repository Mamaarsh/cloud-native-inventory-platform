    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_340px]">
          ...
          <div className="divide-y divide-slate-100">
            {order.data.items.map((item) => (
              <div key={item.id} className="flex flex-col justify-between gap-4 px-6 py-5 sm:flex-row sm:items-center">
                <div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-600"><Package className="size-5" /></span><div><p className="font-bold text-slate-900">{item.product.name}</p><p className="mt-1 font-mono text-xs text-slate-400">{item.product.sku}</p><p className="mt-2 flex items-center gap-1.5 text-xs text-slate-500"><MapPin className="size-3.5" /> {item.warehouse.name} · {item.warehouse.location}</p></div></div>
                <div className="flex items-center justify-between gap-8 pl-15 sm:block sm:pl-0 sm:text-right"><div><p className="text-xs text-slate-400">{formatCurrency(item.unit_price)} × {item.quantity}</p><p className="mt-1 font-bold text-slate-900">{formatCurrency(Number(item.unit_price) * item.quantity)}</p></div></div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          <Card className="p-6"><h3 className="font-bold text-slate-950">Status workflow</h3><p className="mt-2 text-sm leading-6 text-slate-500">Only transitions allowed for your role and the current state are shown.</p>{transitions.length > 0 ? <div className="mt-5 space-y-2">{transitions.map((status) => <Button key={status} variant={status === OrderStatus.Cancelled ? "danger" : "primary"} className="w-full" isLoading={statusMutation.isPending} onClick={() => void changeStatus(status)}>Mark as {titleCase(status)}</Button>)}</div> : <p className="mt-5 rounded-xl bg-slate-100 p-3 text-sm text-slate-600">No status actions are available.</p>}</Card>
      ...
      <Modal open={payOpen} onClose={() => setPayOpen(false)} title="Confirm payment" description="The backend payment service is idempotent for successful payments." size="sm">
        <p className="text-sm leading-6 text-slate-600">Process a payment of <strong>{formatCurrency(total)}</strong> for order #{orderId}?</p>
        <div className="mt-6 flex justify-end gap-3"><Button variant="outline" onClick={() => setPayOpen(false)}>Cancel</Button><Button isLoading={paymentMutation.isPending} onClick={() => void pay()}><CreditCard className="size-4" /> Process payment</Button></div>
      </Modal>
    </>
  );
}
